...
def make_conv2d(x, name, input_depth, kernel_size, n_kernel, pool_size):
    w_size = kernel_size + [input_depth] + [n_kernel]
    w = tf.truncated_normal(w_size, stddev=0.1)
    w = tf.get_variable(name + "_weight", initializer=w)
    b_size = [n_kernel]
    b = tf.constant(0.1, shape=b_size)
    b = tf.get_variable(name + "_bias", initializer=b)
...
def make_dense(x, name, n_input, n_output):
...
def make_classifier(x, y, func, learning_rate):
    y_out = func(x)
